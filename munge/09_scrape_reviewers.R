...
while(count <= nrow(lowered_output)){
  ...
  start_index <- grep("course reviewe", lowered_output[count, ])[1]+1 #+1 for column following "course summary"
  #find first instance of "intended audience"
  end_index <- grep("intended audience", lowered_output[count, ])[1]-1 #-1 for column preceeding "learning outcome"
      ...
      print(paste("Output saved is", output_dataframe[count, start_index]))
        ...
        print(paste("Output saved is", output_dataframe[count, end_index]))
          ...
          print(paste("Output saved is", output_dataframe[count, start_index]))
            ...
            course_reviewer <- c(course_reviewer, paste(output_dataframe[count, (start_index):(end_index)], collapse = " "))
