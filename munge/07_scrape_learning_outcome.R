#function of script: extract course LO
...
while(count <= nrow(lowered_output)){
  ...
  if (length(start_index) == 0 | is.na(start_index) &
    ...
    print(paste("end index not found for iteration ", count))
      ...
      course_objectives <- c(course_objectives, output_dataframe[count, start_index])
      ...
      print(paste("Output saved is", output_dataframe[count, start_index]))
        ...
        course_objectives <- c(course_objectives, output_dataframe[count, end_index])
        ...
        print(paste("Output saved is", output_dataframe[count, end_index]))
          ...
          print(paste("Output saved is", output_dataframe[count, start_index]))
            ...
            course_objectives <- c(course_objectives, paste(output_dataframe[count, (start_index):(end_index)], collapse = " "))
...
output_dataframe$course_objective <- course_objectives
