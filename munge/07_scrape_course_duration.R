...
while(count <= nrow(lowered_output)){
  ...
  start_index <- grep("course duration", lowered_output[count, ])[1]+1 #+1 for column following "course summary"
  #find first instance of "course summary"
  end_index <- grep("course summary", lowered_output[count, ])[1]-1 #-1 for column preceeding "learning outcome"
  ...
  if (length(start_index) == 0 | is.na(start_index) &
    ...
    print(paste("end index not found for iteration ", count))
      ...
      course_durations <- c(course_durations, output_dataframe[count, start_index])
