...
# Need to keep an eye on the per_page query parameter. Currently reads 69 repos. 

 request_result <- GET("https://api.github.com/users/datasciencecampus/repos?per_page=100",
            gtoken,
...
course_repo_names <- unique(lapply(dsca_pages, extract_repo_names) %>% unlist(), fromLast = TRUE)
...
output_dataframe <- as.data.frame(cbind.fill(output_dataframe, dsca_pages))
