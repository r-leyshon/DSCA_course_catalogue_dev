#function of script: extract course lead
...
count <- 1
course_developer <- character()

#continue looping until every row iterated
while(count <= nrow(lowered_output)){
  #find first instance of "lead developer"
  start_index <- grep("lead developer", lowered_output[count, ])[1]+1 #+1 for column following "course summary"
  #find first instance of "course reviewe"
  end_index <- grep("course reviewe", lowered_output[count, ])[1]-1 #-1 for column preceeding "learning outcome"
  ...
  if (length(start_index) == 0 | is.na(start_index) &
    ...
    course_developer <- c(course_developer, "lead developer not found")
    ...
    print(paste("end index not found for iteration ", count))
      ...
      course_developer <- c(course_developer, output_dataframe[count, start_index])
      ...
      print(paste("Output saved is", output_dataframe[count, start_index]))
        ...
        course_developer <- c(course_developer, output_dataframe[count, end_index])
        ...
        print(paste("Output saved is", output_dataframe[count, end_index]))
          ...
          print(paste("Output saved is", output_dataframe[count, start_index]))
            ...
            course_developer <- c(course_developer, paste(output_dataframe[count, (start_index):(end_index)], collapse = " "))
...
output_dataframe$lead_developer <- course_developer
