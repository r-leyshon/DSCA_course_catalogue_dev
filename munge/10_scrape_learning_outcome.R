...
while (count <= nrow(lowered_output)) {
  ...
  start_index <- grep("course objective", lowered_output[count, ])[1] + 1
  #+1 for column following "course summary"
  # find first instance of "lead developer"
  end_index <- grep("lead developer", lowered_output[count, ])[1] - 1
  ...
  if (length(start_index) == 0 | is.na(start_index) &
    ...
    course_objectives <- c(course_objectives,
                           output_dataframe[count, end_index])
    ...
    print(paste("Output saved is",
                paste(output_dataframe[count, start_index:end_index],
                      collapse = " ")))
  }
